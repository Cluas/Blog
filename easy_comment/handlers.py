        ...
        try:
            if not (hasattr(user, "onlinestatus") and user.onlinestatus.is_online()):
                context = {
                    ...
                    "unsend_count": user.notifications.filter(
                        unread=True, emailed=False
                    ).count(),
                    "notice_list": user.notifications.filter(
                        unread=True, emailed=False
                    ),
                    "unread_link": "http://www.cluas.me/notifications/unread/",
                }
                msg_plain = render_to_string(
                    "notifications/email/email.txt", context=context
                )
                send_mail(
                    "来自[Cluas's Blog] 您有未读的评论通知",
                    msg_plain,
...
def like_handler(sender, instance, created, **kwargs):
    if created:
        recipient = ADMINS.exclude(id=instance.user.id).exclude(
            id=instance.comment.user.id
        )
        ...
        if (
            not instance.user.username == instance.comment.user_name
        ) and instance.status:
            notify.send(
                instance.user,
